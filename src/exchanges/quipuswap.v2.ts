...
export const cache = () => {
  return (_target: Object, propertyKey: string, descriptor: PropertyDescriptor) => {
    ...
    descriptor.value = async function (...args: any[]) {
      const exchange = this as QuipuswapV2Exchange
...
export class QuipuswapV2Exchange extends Exchange {
  ...
  public readonly dexType: DexType = DexType.QUIPUSWAP_V2

  public fee: number = 0.997

  constructor(tezos: TezosToolkit, dexAddress: string, pairId: number, token1: Token, token2: Token) {
    super(tezos, dexAddress, token1, token2)
    this.pairId = pairId
  }

  public async token1ToToken2(_tokenAmount: BigNumber, _minimumReceived: BigNumber): Promise<string> {
  ...
  public async token2ToToken1(_tokenAmount: BigNumber, _minimumReceived: BigNumber): Promise<string> {
  ...
  public async getExchangeRate(): Promise<BigNumber> {
    ...
    const value = await this.getStorageValue(storage.storage, 'pairs', this.pairId)

    return new BigNumber(value.token_a_pool.shiftedBy(-1 * this.token1.decimals)).dividedBy(
      new BigNumber(value.token_b_pool).shiftedBy(-1 * this.token2.decimals)
    )
  }
